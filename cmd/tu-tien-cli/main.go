...
func main() {
	...
	session, cleanup, err := buildSession(context.Background(), *configPath, *name)
	if err != nil {
	...
	if err := runTUI(context.Background(), session, cfg.LLM.Model); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}
...
func buildSession(ctx context.Context, configPath string, name string) (*orchestrator.Session, func(), error) {
	cfg, err := config.Load(configPath)
	if err != nil {
